...
void test()
{
    ...
    assert(!ss.get(2, x) && x == "roti");    // get() 2nd value should fail and x is not changed
    assert(!ss.get(-1, x) && x == "roti");    // get() with negative index should fail and x is not changed
    ...
    // test unite where result is alias of s1
    unite(a, b, a);
    assert(a.size() == 3);
    assert(a.contains("pizza"));
    assert(a.contains("ramen"));
    ...
    // test unite where result is alias of s2
    a.erase("ramen");
    unite(a, b, b);
    assert(b.size() == 3);
    assert(b.contains("pizza"));
    ...
    assert(f.size() == 0); // empty - empty = empty

    d.insert("mango");  // s1 = mango
    f.insert("banana");
    excludeInclude(d, e, f);
    assert(f.size() == 0); // empty - non-empty = empty
    ...
    // e: "apple", "mango"
    // f: "apple"
    excludeInclude(f, f, f);
    assert(f.empty());

    // test excludeInclude where result is alias of s1
    Set& g = d;
    excludeInclude(d, e, g); // g is alias of d: d will be changed
    assert(d.size() == 1);
    assert(g.size() == 1);
    assert(d.contains("apple"));
    assert(g.contains("apple"));

    // test excludeInclude where result is alias of s2
    d.insert("melon"); // d: "melon", "apple"; e: "apple", "mango"
    excludeInclude(d, e, e); // ("apple", "mango") - ("melon", "apple") = ("mango")
    assert(e.size() == 1);
    assert(e.contains("mango"));

    excludeInclude(d, g, g); // g is alias of d: d will be changed
    assert(d.empty());
    assert(g.empty());
}
